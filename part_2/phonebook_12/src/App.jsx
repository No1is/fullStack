      ...
      if (updateNumber) {
        ...
        if (confirm) {
          numberService.update(changePerson.id, changePerson).then(returnedPerson => {
            ...
            setNewNumber('')
            setNewName('')
            setMessage(`Number updated for ${changePerson.name} to ${changePerson.number}`)
            setTimeout(() => {
            setMessage(null)
            }, 5000)
          }).catch(error => {
            setError(`Information of ${changePerson.name} has already been removed from server`)
            setTimeout(() => {
              setError(null)
            },5000)
          })
        ...
        setError(`${trimmedName} is already added to the phonebook`)
        setTimeout(() => {
          setError(null)
        },5000)
        setNewNumber('')
  ...
  const handleDelete = (id) => {
    ...
    if (confirm) {
      numberService.remove(id).then(() => {
        ...
        setMessage(`${person.name} has been remove`)
        setTimeout(() => {
        setMessage(null)
        },5000)
      }).catch(error => {
